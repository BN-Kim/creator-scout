...
스카우트 실행은 스카우팅 목표만 입력해 시작할 수 있고 자동 검색이 기본입니다. 자동 검색은 최대 1분 동안 진행되며 제한에 도달하면 그 시점까지 판정과 저장이 완료된 결과만 반환합니다. 검색 설정에서 자동 검색어만, 자동 검색어 + 추가 검색어, 추가 검색어만 모드를 선택할 수 있습니다. 서버의 검증된 분류체계가 여러 승인 카테고리와 좁은·중간·넓은 범위를 순환합니다. 쿼리 진행·점수·연속 페이지·냉각·소진 상태는 SQLite에 저장되며 검색어 점수는 판정 규칙을 바꾸지 않습니다.
...
- `/runs/new` — 새 자동 추천 실행
- `/runs/[id]` — 실행 결과 상세
- `/history` — 크리에이터 히스토리와 3필드 히스토리 내보내기
