  ...
  return <>
    ...
    <aside className={`${open ? "flex" : "hidden"} fixed inset-x-0 top-16 z-20 h-[calc(100vh-4rem)] flex-col border-r border-slate-200 bg-white p-5 lg:fixed lg:inset-y-0 lg:flex lg:h-auto lg:w-64`}>
      ...
      <nav className="space-y-1" aria-label="주요 메뉴">
        {navItems.map((item) => {
          const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href.split("/").slice(0, 3).join("/"));
      ...
      <div className="mt-auto rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500"><strong className="block text-slate-700">목 데이터 환경</strong>외부 API와 연결되지 않은 1단계 MVP입니다.</div>
    </aside>
  </>;
}
