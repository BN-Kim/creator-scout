...
export default function HistoryPage(): React.ReactNode {
  ...
  return <><PageHeader title="크리에이터 히스토리" description="SQLite에 저장된 실제 자동 판정 기록을 한국시간 기준으로 확인합니다." action={<button type="button" className="button-secondary" onClick={download}><Icon name="download" className="h-4 w-4" />JSON 내보내기</button>} />{error && <div role="alert" className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{error}</div>}<div className="panel mb-5 grid gap-3 p-4 md:grid-cols-[1fr_180px_180px]"><label className="relative"><span className="sr-only">채널명 검색</span><Icon name="search" className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input className="input pl-9" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="채널명 검색" /></label><select aria-label="상태 필터" className="input" value={status} onChange={(e) => setStatus(e.target.value as HistoryStatus | "all")}><option value="all">모든 상태</option>{Object.entries(historyStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select aria-label="카테고리 필터" className="input" value={category} onChange={(e) => setCategory(e.target.value)}><option value="all">모든 카테고리</option>{creatorCategories.map((item) => <option key={item}>{item}</option>)}</select></div><div className="panel overflow-hidden"><div className="border-b border-slate-200 p-5 text-sm text-slate-500">총 <strong className="text-ink">{results.length}</strong>개 기록 · JSON은 호환되는 3개 필드만 내보냅니다.</div><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-slate-50 text-xs font-semibold text-slate-500"><tr><SortHeader label="채널명" active={sort === "channelName"} ascending={ascending} onClick={() => changeSort("channelName")} /><th className="px-4 py-3">히스토리 상태</th><th className="px-4 py-3">최종 판정</th><SortHeader label="카테고리" active={sort === "category"} ascending={ascending} onClick={() => changeSort("category")} /><SortHeader label="판정 시각(KST)" active={sort === "processedAt"} ascending={ascending} onClick={() => changeSort("processedAt")} /><th className="px-4 py-3">판정 사유</th><th className="px-4 py-3">추천 실행</th></tr></thead><tbody className="divide-y divide-slate-100">{results.map((record) => <tr key={record.id} className="hover:bg-slate-50"><td className="px-4 py-4"><p className="font-semibold text-ink">{record.identity.channelName}</p>{record.identity.canonicalChannelUrl ? <a href={record.identity.canonicalChannelUrl} target="_blank" rel="noreferrer" className="mt-1 block text-xs text-brand hover:underline">확인된 채널 URL ↗</a> : <span className="text-xs text-slate-400">URL 미확인</span>}</td><td className="px-4 py-4"><StatusBadge status={record.historyStatus} /></td><td className="px-4 py-4"><StatusBadge status={record.finalDecision} /></td><td className="px-4 py-4 text-slate-600">{record.category}</td><td className="px-4 py-4 whitespace-nowrap text-slate-500">{formatDateTime(record.updatedAt)}</td><td className="max-w-sm px-4 py-4 text-xs leading-5 text-slate-500">{record.koreanExplanation}</td><td className="px-4 py-4 font-mono text-xs text-slate-500">{record.scoutingRunId}</td></tr>)}</tbody></table></div></div></>;
}
