...
import { MarketingOutcomeControls } from "./marketing-outcome-controls";
...
function HistoryRow({ record }: { record: HistoryRecord }): React.ReactNode {
  ...
  return <tr className="hover:bg-slate-50"><td className="px-4 py-4"><p className="font-semibold text-ink">{record.identity.channelName}</p>{record.identity.canonicalChannelUrl ? <a href={record.identity.canonicalChannelUrl} target="_blank" rel="noreferrer" className="mt-1 block text-xs text-brand hover:underline">확인된 채널 URL ↗</a> : <span className="text-xs text-slate-400">URL 미확인</span>}</td><td className="px-4 py-4"><StatusBadge status={record.historyStatus} />{record.decisionSource === "manual" && <span className="mt-1 block text-[11px] font-semibold text-blue-600">수동</span>}</td><td className="px-4 py-4 text-slate-600">{record.category}</td><td className="w-32 px-4 py-4 text-slate-500"><time dateTime={record.updatedAt}><span className="block whitespace-nowrap">{executedAt.date}</span><span className="mt-0.5 block whitespace-nowrap">{executedAt.time}</span></time></td><td className="max-w-sm px-4 py-4 text-xs leading-5 text-slate-500"><div className="space-y-1">{formatHistoryReasonLines(record.koreanExplanation, record.reasonCodes, record.finalDecision).map((line, index) => <p key={`${record.id}-reason-${index}`}>{line}</p>)}</div></td><td className="px-4 py-4 font-mono text-xs text-slate-500">{record.scoutingRunId}</td><td className="px-4 py-4 align-top"><MarketingOutcomeControls historyRecordId={record.id} /></td></tr>;
}
