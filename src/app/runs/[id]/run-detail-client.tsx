...
export function RunDetailClient({ runId }: { runId: string }): React.ReactNode {
  ...
  const evaluateAndSave = useCallback(async (corrections: Record<string, ManualCorrection> = {}): Promise<void> => {
    const repository = new HistoryApiClient();
    await migrateBrowserHistory(window.localStorage, repository);
  ...
  useEffect(() => { void evaluateAndSave().catch((error: unknown) => { console.error("추천 실행 히스토리를 저장하지 못했습니다.", error); setMessage("서버 히스토리를 처리하지 못했습니다. 잠시 후 다시 시도해 주세요."); }); }, [evaluateAndSave]);
  ...
  const correct = (code: ManualCorrectionCode, note: string): void => {
    ...
    const selectedName = selected.identity.channelName;
    void evaluateAndSave(corrections).then(() => {
      setSelectedId(null); setMessage(`${selectedName}을(를) 제외로 교정하고 히스토리에 반영했습니다.`);
    }).catch((error: unknown) => {
      console.error("수동 교정을 저장하지 못했습니다.", error);
      setMessage("수동 교정을 서버 히스토리에 저장하지 못했습니다.");
    });
  };
