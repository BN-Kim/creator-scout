...
H5.1은 공식 Data API가 아니라 서버 전용 YouTube.js 브리지에서 정확한 채널의 공개 설명, 직접 공개된 외부 링크와 최근 공개 영상 최대 20개의 제목·설명을 별도 안정 스냅샷으로 수집합니다. 게이트된 “이메일 주소 보기”는 호출하지 않습니다. 이 스냅샷은 기존 H5 리크루팅 정규화에만 전달되며 완전한 YouTube.js 응답은 저장하거나 노출하지 않습니다.
...
공급자 계층은 H4 파이프라인의 중복 선검사, 실행 통계, 평가와 자동 저장을 그대로 재사용합니다. H5.1 공개 연락·소속 수집은 결정 008의 범위에서 별도 리크루팅 공급자가 담당합니다. 실제 시청자 지역 근거, 스케줄링과 운영 제어는 포함되지 않습니다.
