...
H5.1 라이브 출처 감사 시점에는 승인된 라이브 출처가 없습니다. 따라서 출처별 라이브 클라이언트나 새 픽스처·목 테스트를 만들지 않으며, 문서 체크포인트 변경은 `npm run verify`로 기존 계약이 유지되는지 확인합니다. 향후 출처가 명시적으로 승인되면 `npm run test:providers`와 `npm run test:e2e`도 별도로 실행해야 합니다.
