...
H4/H4.2 자동 파이프라인은 `tests/scouting/automatic-scouting-pipeline.test.ts`의 19개 테스트에서 목 공급자만 사용해 비용 순서, 중복 건너뛰기, 세 판정 자동 저장, 추천 목표 충족, 다중 페이지 발견, 목표 초과 방지, 소스 소진, 후보·페이지·시간·공급자 실패 안전 상한, 반복 실행 멱등성과 후보별 공급자·정규화·저장 실패 격리를 검증합니다. Playwright는 새 추천 실행 제출, 목표 대비 충족 수와 부분 완료 사유를 실제 키 없이 확인합니다.
