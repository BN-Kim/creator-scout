...
히스토리는 사용자가 공급하거나 관리하는 파일이 아니라 애플리케이션이 확정 판정마다 자동 생성하는 내부 기억입니다. 자동 파이프라인은 안정적인 신원을 확보한 직후 SQLite를 확인하고, 과거 일치 채널을 근거 수집과 판정 전에 조용히 건너뜁니다. 건너뛴 채널은 현재 결과나 새 히스토리 레코드가 되지 않으며 실행 통계나 로그에만 집계합니다. 동일 실행 신원 블록리스트도 판정 전 적용합니다.

신규 채널의 모든 확정 판정은 자동 저장합니다. 저장 경계의 반복·경합 쓰기는 새 레코드를 만들지 않고 동일 신원 레코드를 유지하며, 더 강한 신원 근거가 확인되면 기존 미확인 필드를 보강합니다. 다운로드 JSON은 `channel_name`, `url`, `status`만 제공하는 선택적 호환 출력이며 저장소 원본이나 입력이 아닙니다. 별도의 과거 히스토리 파일, 업로드 또는 가져오기 작업은 필요하지 않습니다.
