...
공식 YouTube Data API v3 응답은 공급자 어댑터에서 정규화합니다. 최신 업로드 최대 50개를 조회하고 설정된 활동 기간 안의 영상 수와 최신 10개까지의 조회수 평균을 계산합니다. 삭제·비공개 영상이나 게시 시각 누락처럼 구간 계산을 완결할 수 없는 응답은 숫자 0으로 바꾸지 않고 미확인 근거로 전달합니다.
...
H4.3은 H4.2 앞단에 `AdaptiveQuerySelector`를 둡니다. 서버에서 검증한 분류체계가 좁은·중간·넓은 한국어 검색어를 결정론적으로 만들고, `automatic`, `manual_replace`, `manual_extend` 모드가 선택 가능한 검색어 집합을 정합니다. 선택기는 실행 중 선택 횟수가 적은 카테고리를 먼저 배치한 뒤 기존 표본·성과 점수를 적용합니다. 공식 API에서 호출당 최대 50명의 후보를 받아 실행 메모리 큐에 두고 검색어별 최대 10명씩 교대 평가합니다. 현재 페이지를 전부 처리한 경우에만 다음 연속 페이지 토큰을 저장하고, 시간·후보 한도 등으로 중단되면 기존 토큰을 유지해 다음 실행에서 그 페이지를 다시 안전하게 확인합니다. 쿼리 본문, 정규화 키, 카테고리, 범위, 연속 페이지 토큰, 시도·페이지·중복·판정·실패 집계, 냉각·소진 상태는 SQLite `discovery_query_state`에 저장됩니다. 검증 카테고리가 검색 카테고리와 일치해 추천된 크리에이터의 공개 메타데이터에서 해당 카테고리 핵심어를 포함하는 문구만 `discovery_learned_terms`에 탐색 상태로 저장하며, 출처 채널 ID·공개 URL·정규화 근거 신호를 함께 보존합니다. 최소 표본 전에는 검증된 상위 검색어가 될 수 없으며 성과가 나쁜 학습 문구는 냉각 또는 폐기됩니다. 이 점수와 상태는 발견 순서만 바꾸고 결정 엔진 입력이나 판정 임계값은 바꾸지 않습니다.
